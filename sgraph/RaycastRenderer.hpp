...
namespace sgraph {
    ...
    public:
         ...
         */
        RaycastRenderer(map<string, util::TextureImage *> &textures, stack<glm::mat4> &mv, vector<util::Light> &lights,
                        string outfileLoc)
                : textures(textures),
                  modelview(mv),
                  lights(lights),
        ...
        void visitLeafNode(LeafNode *leafNode) {
            ...
            if (modelType != "box" && modelType != "sphere"
                && modelType != "cylinder" && modelType != "cone")
                return;
        ...
        void raycastSphere(Ray3D &objSpaceRay, HitRecord &hit, RaycastObj &obj) {
            ...
            float B = 2.f *
                      (objSpaceRay.direction.x * objSpaceRay.start.x + objSpaceRay.direction.y * objSpaceRay.start.y +
                       objSpaceRay.direction.z * objSpaceRay.start.z);
            float C = objSpaceRay.start.x * objSpaceRay.start.x + objSpaceRay.start.y * objSpaceRay.start.y +
        ...
        void raycastCylinder(Ray3D &ray, Ray3D &objSpaceRay, HitRecord &hit, RaycastObj &obj) {
            ...
            else if (objSpaceIntersection.y > 0.999f) {
                hit.texCoord = glm::vec2(objSpaceIntersection.x * 0.25f + 0.75f, objSpaceIntersection.z * 0.25f + 0.75f);
            }
            else
            {
                hit.texCoord = glm::vec2(theta / glm::radians(360.0f),
            ...
            for (auto &light: lights) {
              ...
              HitRecord shadowcastHit;
                ...
                if (shadowcastHit.time >= 1.f || shadowcastHit.time < 0) {
                    ...
                    if (nDotL > 0)
                        specular = compMul(hit.mat->getSpecular(), light.getSpecular()) *
                                   pow(rDotV, max(hit.mat->getShininess(), 1.f));
                } else {
                    diffuse = {0., 0., 0.};
                    specular = {0., 0., 0.};
                }
