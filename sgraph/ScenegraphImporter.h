...
namespace sgraph {
        ...
        IScenegraph *parse(istream &input) {
            ...
            while (inputWithOutComments >> command) {
                ...
                if (command == "instance") {
                    ...
                    imgLoader->load(path);
                    textures[name] = new util::TextureImage(imgLoader->getPixels(),imgLoader->getWidth(),imgLoader->getHeight(),name);
                    ...
                    parseAddLight(inputWithOutComments);
    ...
    protected:
        ...
        virtual void parseLeaf(istream &input) {
            string varname, name, command, leaftype, textureName;
            input >> varname >> name;
            cout << "Read " << varname << " " << name << endl;

            input >> command; // command = instanceof
            input >> leaftype;
            SGNode *leaf = new LeafNode(leaftype, name, NULL);
            nodes[varname] = leaf;
        }
        ...
        virtual void parseAddLight(istream &input) {
        ...
        virtual void parseAssignTexture(istream &input) {
            ...
            if ((leafNode != NULL) && (textures.find(texname) != textures.end())) {
                leafNode->setTextureName(texname);
            }
        }
